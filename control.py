...
def batch_features(batch: torch.Tensor) -> torch.Tensor:
...
def features(dataset: RadiographDataset, batch_size: int = 256) -> np.ndarray:
    rows = []
    for images, _labels, _paths in DataLoader(dataset, batch_size=batch_size):
        rows.append(batch_features(images).numpy())
...
def main() -> int:
    ...
    validation = RadiographDataset.labeled(split_dir(root, "val"), args.image_size)
    test = RadiographDataset.labeled(split_dir(root, "test"), args.image_size)
    train_features = features(train)
    validation_features = features(validation)
    test_features = features(test)
    validation_labels = np.asarray(validation.labels)
    ...
    results = []
    candidates = [
        (name, validation_z[:, index], test_z[:, index])
